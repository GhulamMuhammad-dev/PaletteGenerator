...
import { Monitor, Smartphone, Sun, Moon, FileText, Settings, Edit3, Type, Image, Square, Upload, X } from 'lucide-react';
      ...
      {showImageUpload && (
        <div className="mb-6 p-4 bg-orange-50 dark:bg-orange-900/20 rounded-lg border border-orange-200 dark:border-orange-800">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            {/* Feature Icons */}
            <div className="space-y-2">
              <label className="block text-xs font-medium text-orange-800 dark:text-orange-300">Feature Icons</label>
              <div className="grid grid-cols-3 gap-1">
                {imageAssets.featureIcons.map((icon, index) => (
                  <div key={index} className="relative">
                    {icon ? (
                      <div className="relative group">
                        <img src={icon} alt={`Feature ${index + 1}`} className="w-full h-12 object-contain bg-gray-100 dark:bg-gray-700 rounded border" />
                        <button
                          onClick={() => removeImage(`featureIcon-${index}`)}
                      ...
                      <button
                        onClick={() => openImageUpload(`featureIcon-${index}`)}
                        className="w-full h-12 border-2 border-dashed border-orange-300 dark:border-orange-600 rounded flex items-center justify-center hover:border-orange-400 dark:hover:border-orange-500 transition-colors"
                      >
        ...
        <div className="mb-6 p-4 bg-green-50 dark:bg-green-900/20 rounded-lg border border-green-200 dark:border-green-800">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-3">
              ...
              <div>
                <label className="block text-xs font-medium text-green-800 dark:text-green-300 mb-1">Features Section Title</label>
                <input
                  type="text"
                  value={customContent.featureTitle}
                  onChange={(e) => setCustomContent(prev => ({ ...prev, featureTitle: e.target.value }))}
                  className="w-full text-sm border border-green-300 dark:border-green-600 rounded px-2 py-1 bg-white dark:bg-gray-700 dark:text-gray-200"
                />
              </div>
              <div>
              ...
              {customContent.features.map((feature, index) => (
                <div key={index} className="space-y-1">
                  <input
                    ...
                    value={feature.title}
                    onChange={(e) => {
                      const newFeatures = [...customContent.features];
                      newFeatures[index].title = e.target.value;
                      setCustomContent(prev => ({ ...prev, features: newFeatures }));
                    }}
                    className="w-full text-xs border border-green-300 dark:border-green-600 rounded px-2 py-1 bg-white dark:bg-gray-700 dark:text-gray-200"
                  />
                  <textarea
                    ...
                    className="w-full text-xs border border-green-300 dark:border-green-600 rounded px-2 py-1 h-12 resize-none bg-white dark:bg-gray-700 dark:text-gray-200"
                  />
                </div>
              ))}
            </div>
